...
<?php
    ...
    $reportgrid->setSQLColumn('b.biblio_id', 'b.title AS \''.__('Title').'\'', 'COUNT(item_id) AS \''.__('Copies').'\'',
		...
		'pl.place_name AS \''.__('TMP Terbit').'\'',
		'b.publish_year AS \''.__('Thn.Terbit').'\'',
		'b.language_id AS \''.__('BHS').'\'',
        'b.isbn_issn AS \''.__('ISBN/ISSN').'\'',
        'b.collation AS \''.__('Collation').'\'',
		'b.input_date AS \''.__('Data Masuk').'\'',
		'b.gmd_id AS \''.__('GMD').'\'',
        'b.call_number AS \''.__('DDC').'\'');
    ...
    $subquery_str = '(SELECT DISTINCT bsub.biblio_id, bsub.gmd_id, bsub.title, bsub.isbn_issn, bsub.call_number, bsub.classification, bsub.language_id,
		bsub.publish_place_id, bsub.publisher_id, bsub.collation, bsub.publish_year, bsub.edition, bsub.input_date
        FROM biblio AS bsub
