...
Author      : Eddy Subratha (eddy.subratha{at}gmail.com), Syam Suryanto (syamsuryanto@gmail.com) Erwan Setyo Budi (erwans818@gmail.com)
   ...
   /* COLLECTION TYPE LIST */
    ...
    if ($can_read AND $can_write) {
        $datagrid->setSQLColumn(
            'ct.read_id', 
            'ct.item_id AS \''.__('Kode Barcode Type').'\'', 
            'b.title AS \''.__('Judul').'\'',
            'ct.update_at AS \''.__('Last Update').'\'');
    } else {
        $datagrid->setSQLColumn('ct.item_id AS \''.__('Kode Item').'\'', 'ct.update_at AS \''.__('Last Update').'\'');
    }
    $datagrid->setSQLorder('ct.item_id ASC');
    ...
    if (isset($_GET['keywords']) AND $_GET['keywords']) {
       ...
       $datagrid->setSQLCriteria("ct.item_id LIKE '%$keywords%'");
    }
    ...
    /* Form Element(s) */
    // coll_type_name
    $form->addTextField('text', 'collTypeName', __('Kode Item').'*', $rec_d['item_id'], 'style="width: 60%;"');
