...
<?php
    ...
    // query Read Counter data to database
    $_visitor_q = $dbs->query("SELECT SUBSTRING(`update_at`, 9, 2) AS `mdate`, COUNT(read_id) AS `vtotal` FROM `read_counter` WHERE `update_at` LIKE '$selected_year-$selected_month%' GROUP BY DATE(`update_at`)");
    ...
    // print out
    echo '<div class="printPageInfo">Read Counter Report for <strong>'.$months[$selected_month].', '.$selected_year.'</strong> <a class="printReport" onclick="window.print()" href="#">'.__('Print Current Page').'</a></div>'."\n";
    echo $output;
