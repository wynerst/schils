...
<?php
    ...
    if ($class_num == 'NONDECIMAL') {
        ...
        foreach ($months as $month_num => $month) {
            $loan_q = $dbs->query("SELECT COUNT(*) FROM biblio AS b
                ...
                LEFT JOIN read_counter AS l ON l.item_id=i.item_code
                WHERE classification REGEXP '^[^0-9]' AND l.update_at LIKE '$selected_year-$month_num-%'".( !empty($coll_type)?" AND i.coll_type_id=$coll_type":'' ));
        ...
        foreach ($months as $month_num => $month) {
            $loan_q = $dbs->query("SELECT COUNT(*) FROM biblio AS b
                LEFT JOIN item AS i ON b.biblio_id=i.biblio_id
                LEFT JOIN read_counter AS l ON l.item_id=i.item_code
                WHERE TRIM(classification) LIKE '$class_num%' AND l.update_at LIKE '$selected_year-$month_num-%'".( !empty($coll_type)?" AND i.coll_type_id=$coll_type":'' ));
            $loan_d = $loan_q->fetch_row();
            if ($loan_d[0] > 0) {
                $output .= '<td class="'.$row_class.'"><strong style="font-size: 1.5em;">'.$loan_d[0].'</strong></td>';
            } else {
        ...
        while ($class_num2 < 10) {
            ...
            foreach ($months as $month_num => $month) {
                $loan_q = $dbs->query("SELECT COUNT(*) FROM biblio AS b
                    LEFT JOIN item AS i ON b.biblio_id=i.biblio_id
                    LEFT JOIN read_counter AS l ON l.item_id=i.item_code
                    WHERE TRIM(classification) LIKE '$class_num"."$class_num2%' AND l.update_at LIKE '$selected_year-$month_num-%'".( !empty($coll_type)?" AND i.coll_type_id=$coll_type":'' ));
                $loan_d = $loan_q->fetch_row();
                if ($loan_d[0] > 0) {
                    $output .= '<td class="'.$row_class.'"><strong style="font-size: 1.5em;">'.$loan_d[0].'</strong></td>';
                } else {
    ...
    // print out
    echo '<div class="printPageInfo">Read Recap By Class <strong>'.$class_num.'</strong> for year <strong>'.$selected_year.'</strong>'.( isset($coll_type_name)?'<div>'.$coll_type_name.'</div>':'' ).' <a class="printReport" onclick="window.print()" href="#">'.__('Print Current Page').'</a>';
